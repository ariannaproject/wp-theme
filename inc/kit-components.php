 ...
 */
function arianna_kit_components_create_relation_table() {
    global $wpdb;
...
add_action( 'after_setup_theme', 'arianna_kit_components_create_relation_table' );
...
function arianna_kit_components_metabox() {
    add_meta_box(
        ...
        'Seleziona i componenti del Kit',           // Title
        'arianna_kit_components_render_metabox',    // Callback function for content
        'kit',                                      // Assign to CPT 'kits'
...
function arianna_kit_components_render_metabox($post) {
    ...
    $components = get_posts(['post_type' => 'component', 'numberposts' => -1, 'orderby' => 'title', 'order' => 'ASC']);
...
add_action('save_post_kit', 'arianna_kit_components_save');


/**
 * Retrieve Components linked to a specific Kit
 */
function arianna_kit_components_get($kit_id) {
    global $wpdb;
    $table_name = $wpdb->prefix . 'kits_components_relations';
...
/**
 * Retrieve Kits linked to a specific Component
 */
function arianna_kit_components_get_kits($component_id) {
    global $wpdb;
    $table_name = $wpdb->prefix . 'kits_components_relations';

    $kit_ids = $wpdb->get_col($wpdb->prepare(
        "SELECT kit_id FROM $table_name WHERE component_id = %d",
        $component_id
    ));
